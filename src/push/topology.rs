...
impl Topology {
    ...
    pub fn euclidean_distance(i1: &Vec<usize>, i2: &Vec<usize>) -> Option<f64> {
        if i1.len() != i2.len() {
            None
        } else {
            let mut dist = 0.0;
            for i in 0..i1.len() {
                dist += (i1[i] as f64 - i2[i] as f64).powf(2.0);
            }
            Some(f64::sqrt(dist))
        }
    }
    ...
    pub fn find_neighbors(
        ...
        let nedge = f64::ceil((*ntotal as f64).powf(1.0 / *ndim as f64)) as usize;
...
mod tests {
    ...
    fn find_neighbors_without_envelope() {
        ...
        assert_eq!(
            Topology::find_neighbors(&27, &3, &13, &f64::sqrt(3.0)).unwrap(),
            IntVector::new(vec![
    ...
    fn find_neighbors_with_envelope() {
        assert_eq!(
            Topology::find_neighbors(&38, &2, &31, &f64::sqrt(2.0)).unwrap(),
